<script>
  ...
  const getComments = async pullRequest => {
    ...
    if (response.value && response.value.length > 0) {
      return response.value.filter(({ identities, isDeleted, comments }) => identities === null && !isDeleted && isNotSystemComments(comments));
    }
  }
  ...
  <div class="skz-pullrequest__details">
    ...
    <div class="skz-pullrequest-infos">
      <div  class="skz-pullrequest__counters">
        <span class="skz-pullrequest__counter skz-pullrequest__counter--approval">{pullRequest.reviewers.filter(({ vote }) => vote === 10).length}</span>
        <span class="skz-pullrequest__counter skz-pullrequest__counter--valid">{pullRequest.reviewers.filter(({ vote }) => vote === 5).length}</span>
        <span class="skz-pullrequest__counter skz-pullrequest__counter--wait">{pullRequest.reviewers.filter(({ vote }) => vote === -5).length}</span>
        <span class="skz-pullrequest__counter skz-pullrequest__counter--reject">{pullRequest.reviewers.filter(({ vote }) => vote === -10).length}</span>
      </div>
