...
{#each themes as themeItem}
  <li class="skz-theme">
    <input id={themeItem.id} type="radio" bind:group={chosenTheme} value={themeItem.id}>
    <label for={themeItem.id} class="skz-theme__gradient skz-theme__gradient--{themeItem.id}">{themeItem.text}</label>
  </li>
{/each}
</ul>