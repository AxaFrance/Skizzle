...
<main>
	<div>
		...
		<div>
			<button
				on:click={() => (tabs = 'azure')}
				disabled={tabs === 'azure'}>Azure</button>
			<button
				on:click={() => (tabs = 'github')}
				disabled={tabs === 'github'}>Github</button>
			{#if $pullRequests.length > 0}
				<button
					on:click={() => (tabs = 'prs')}
					disabled={tabs === 'prs'}>PullRequests</button>
			{/if}
		</div>
		{#if $isLoading}
			<p>Chargement...</p>
		{:else if tabs === 'azure'}
			<button
				on:click={() => authorize(ProviderEnum.AzureDevOps)}
