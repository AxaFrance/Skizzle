<script>
	...
	$: searchableProject = projects.filter(
		p =>
			p.name.toLowerCase().indexOf(searchInput.toLowerCase()) !== -1 ||
			p.repositories.filter(
				r => r.name.toLowerCase().indexOf(searchInput.toLowerCase()) !== -1,
			).length > 0,
	);
	...
	<div class="skz-projects__list-container">
		<ul class="skz-projects__list">
			{#each searchableProject as project}
				<Project {project} checked={checked && canCheckedProject(project)} />
			{:else}
				<li>
