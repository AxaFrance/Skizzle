<script lang="ts">
  ...
  import { profiles, customLists, pullRequests, repositories, settings } from 'shared/stores/default.store';
  ...
  .filter(pr => customList.tags.length === 0 || customList.tags.some(x => (pr.labels ?? []).some(y => y.name === x)))
  .filter(pr => {
    let exist = !customList.withoutOwnedByUserPR;
    const profile = $profiles.find(x => x.provider === pr.provider);

    if (profile) {
      console.log({ exist, profile, user: pr.user });
      exist = exist || (customList.withoutOwnedByUserPR && pr.user.id && profile.id !== pr.user.id);
    }

    return exist;
  })
  .filter(pr => !customList.withoutDraft || (customList.withoutDraft && pr.isDraft))
  .filter(pr => !customList.withoutDraft || (customList.withoutDraft && pr.isDraft))
  .filter(pr => !customList.withoutDraft || (customList.withoutDraft && !pr.isDraft))
  .filter(pr => !customList.withoutDraft || (customList.withoutDraft && pr.isDraft)) as PullRequestType[];
</script>
