...
<main>
	<div class={`step ${step === 0 ? 'current' : ''}`}>
		...
		<p>
			Skizzle vous permet de regrouper toutes les pull requests de vos projets
			issus de Github ou Azure DevOps.
		</p>
		<p>
			Nous allons maintenant configurer ensemble l'application rapidement en
			quelques étapes. Vous êtes prêt ?
		</p>
		<div class="actions">
	...
	<div class={`step ${step === 1 ? 'current' : ''}`}>
		...
		<p>
			Skizzle supporte Github et Azure DevOps, pour pouvoir utiliser l'application
			vous devez ajouter au moins un compte. Choisissez une option et laissez-vous
			guider.
		</p>
		<ul class="block">
	...
	<div class={`step ${step === 2 ? 'current' : ''}`}>
		...
		<p>
			Suivez les repositories qui vous intéressent, vous verrez alors leurs pull
			requests dans vos listes.
		</p>
		<div class="block">
			<Tabs
